  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-semibold text-urban-gray-light">Nova Escola</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-card bg-sidebar/80 border border-urban-petrol/30 p-6 space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Ex.: EMEF..." className={inputClass} />
          </div>
          <div />
          <div>
          ...
          <div>
            ...
            <input type="text" value={ibgeCodeDisplay} readOnly placeholder="Preenchido pela seleção" className={inputClass + ' bg-white/5 cursor-not-allowed'} />
          </div>
          <div>
          ...
          <div>
            <label className={labelClass}>Diretor(a)</label>
            <input type="text" value={principal} onChange={(e) => setPrincipal(e.target.value)} placeholder="Nome do diretor" className={inputClass} />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={status} onChange={(e) => setStatus(e.target.value as School['status'])} className={inputClass}>
          ...
          <div>
            ...
            <input type="text" inputMode="decimal" value={lat} readOnly placeholder={loadingGeocode ? 'Buscando...' : 'Preenchido pelo endereço'} className={inputClass + ' bg-white/5 cursor-not-allowed'} />
          </div>
          <div>
            <label className={labelClass}>Longitude</label>
            <input type="text" inputMode="decimal" value={lng} readOnly placeholder={loadingGeocode ? 'Buscando...' : 'Preenchido pelo endereço'} className={inputClass + ' bg-white/5 cursor-not-allowed'} />
          </div>
        </div>
        <div className="flex gap-3 pt-4 border-t border-urban-petrol/30">
          <button type="submit" className="px-4 py-2 rounded-lg bg-urban-green hover:bg-urban-green-medium text-white font-medium transition-colors">Cadastrar escola</button>
          <Link to="/escolas" className="px-4 py-2 rounded-lg bg-white/10 text-urban-gray-light hover:bg-white/20 font-medium transition-colors">Cancelar</Link>
        </div>
      </form>
    </div>
  );
